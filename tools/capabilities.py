...
Usage:
    python tools/capabilities.py            # print the index
    python tools/capabilities.py --write    # refresh CAPABILITIES.md
    python tools/capabilities.py --check    # exit 1 if CAPABILITIES.md is stale
"""
...
def collect_integrations() -> dict[str, list[dict]]:
    """Return catalogued MCP servers and companion skills, if PyYAML is present."""
    manifest = ROOT / "templates" / ".agent" / "integrations.yml"
    if not manifest.exists():
        return {"mcp": [], "skills": []}
    try:
        import yaml
    except ImportError:  # pragma: no cover - optional at generation time
        return {"mcp": [], "skills": []}
    data = yaml.safe_load(manifest.read_text(encoding="utf-8"))
    return {
        "mcp": [
            {"name": n, "enables": _first_sentence(s.get("enables", ""))}
            for n, s in (data.get("mcp") or {}).items()
        ],
        "skills": [
            {"name": n, "enables": _first_sentence(s.get("enables", ""))}
            for n, s in (data.get("skills") or {}).items()
        ],
    }
...
def render() -> str:
    ...
    out: list[str] = [
        ...
        "",
        "Generated by `python tools/capabilities.py` from live repo state — never",
        "hand-edit. Adding a phase, reference or tool makes it appear here.",
        "",
        "## Ways to invoke",
        "",
